                        ...
                        <tbody>
                            @foreach ($sliders as $slider)
                                <tr>
                                    <td>{{ $loop->index + 1 }}</td>
                                    <td>
                                        <img class="img-circle img-bordered-sm" src="{{ $slider->photo }}"
                                            width="80" alt="Team Work Image">
                                    </td>
                                    <td class="text-center">
                                        <div class="btn-group" role="group" aria-label="Basic outlined example">
                                            <a href="{{ route('panel.programming.sliders.edit', $slider->id) }}"
                                                class="btn btn-outline-primary">
                                                <i class="fa-regular fa-pen-to-square"></i>
                                            </a>
                                            <form action="{{ route('panel.programming.sliders.destroy', $slider->id) }}"
                                                method="POST">
                                                @csrf
                                                @method('DELETE')
